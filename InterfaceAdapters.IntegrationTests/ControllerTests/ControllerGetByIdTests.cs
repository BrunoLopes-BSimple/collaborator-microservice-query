...
public class ControllerGetByIdTests : IntegrationTestBase, IClassFixture<IntegrationTestsWebApplicationFactory<Program>>
{
    private readonly IntegrationTestsWebApplicationFactory<Program> _factory;
    public ControllerGetByIdTests(IntegrationTestsWebApplicationFactory<Program> factory) : base(factory.CreateClient())
    { _factory = factory; }

    [Fact]
    public async Task GetById_ShouldReturnSuccessWhenIdIsFound()
    {
        // arrange
        ...
        // act
        var response = await GetAndDeserializeAsync<CollaboratorDTO>($"api/collaborators/{collabId1}");

        // assert
        Assert.NotNull(response);
        Assert.Equal(collabId1, response.Id);
    }

    [Fact]
    public async Task GetById_ShouldReturnNotFoundWhenIdDoesNotExist()
    {
        // arrange
        var nonExistentId = Guid.NewGuid();

        // act
        var response = await GetAsync($"api/collaborators/{nonExistentId}");
